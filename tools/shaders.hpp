...
namespace STP3D {
	...
	*/
	...
	void ShaderManager::printLog(GLuint object, bool isShader, const char* str) {
		GLsizei length = 0;
	...
	GLuint ShaderManager::loadShader(const char *vertexFile, const char *fragmentFile, bool v) {
	...
	GLuint ShaderManager::loadShader(const std::vector<const char *> filenames, const std::vector<ShaderType> shaderTypes, bool v) {
		GLuint programObject;
		if(v) std::cout << "Begin initializing shaders" << std::endl;
		CHECK_GL;
	...
	bool ShaderManager::linkProgram(GLuint programObject, bool verbose) {
	...
	bool ShaderManager::compileShader(const char *shaderFile, const ShaderType shaderType, GLuint& programObject, bool verbose) {
		// Vertex shader
		GLchar *shaderSource;
	...
	bool ShaderManager::loadSource(const char* filename, char** source) {
	...
	void ShaderManager::deleteProgram(GLuint programObject) {
		// S'il existe on supprime le programme GLSL
		if(programObject) glDeleteObjectARB(programObject);
	}

	bool ShaderManager::areShadersSupported(bool v = false) {
		if (v) {
	...
	std::string ShaderManager::writeShaderType(ShaderType shdtype) {
		switch(shdtype) {
			case(Vertex) : 
				return "Vertex";
			case(Fragment) : 
	...
	GLenum ShaderManager::convertToGLShaderType(ShaderType shdtype) {
		switch(shdtype) {
			case(Vertex) : 
