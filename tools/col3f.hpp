...
namespace STP3D {
	...
	static inline  bool g2x_FillColorMap(Col3f *map, int n)
	{
		...
		for (u=0;u<n;u++)
		{  
			h = (360.*std::log(1.+ u*Dh))*DEGtoNG;
			/*	h=((360.*u)/n)*DEGtoNG; */
			x = SQR(h-(int)h);
			switch((int)h)
			{
				case 0  : map[u] = Col3f(1. ,x,   0.); break; 
				case 1  : map[u] = Col3f(1-x,1.,  0.); break;
				case 2  : map[u] = Col3f(0. ,1.,  x ); break;
				case 3  : map[u] = Col3f(0. ,1.-x,1.); break; 
				case 4  : map[u] = Col3f(x  ,0.,  1.); break;
				default : map[u] = Col3f(1. ,0.,1.-x); break;
			}
			//std::cerr<<"Map "<<u<<" : "<<map[u]<<std::endl;
		}
	 	return true; 
	}
